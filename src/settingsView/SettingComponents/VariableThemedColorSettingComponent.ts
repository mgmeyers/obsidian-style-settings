...
export class VariableThemedColorSettingComponent extends AbstractSettingComponent {
	...
	render(containerEl: HTMLElement): void {
		...
		if (typeof this.setting["default-light"] !== "string" || !isValidDefaultColor(this.setting["default-light"])) {
			return console.error(
				`${t("Error:")} ${title} ${t("missing default light value, or value is not in a valid color format")}`,
			);
		}

		if (typeof this.setting["default-dark"] !== "string" || !isValidDefaultColor(this.setting["default-dark"])) {
			return console.error(
		...
		const wrapper = this.settingEl.controlEl.createDiv({cls: "themed-color-wrapper"});
	...
	private createColorPickerLight(wrapper: HTMLDivElement, containerEl: HTMLElement, swatchesLight: string[], valueLight: number | string | boolean, idLight: string) {
		...
		this.pickrLight.on("save", (color: Pickr.HSVaColor, instance: Pickr) => this.onSave(idLight, color, instance));

		this.pickrLight.on("cancel", onPickrCancel);

		const themeLightReset = new ButtonComponent(themeLightWrapper.createDiv({cls: "pickr-reset"}));
	...
	private createColorPickerDark(wrapper: HTMLDivElement, containerEl: HTMLElement, swatchesDark: string[], valueDark: number | string | boolean, idDark: string) {
		...
		this.pickrDark.on("save", (color: Pickr.HSVaColor, instance: Pickr) => this.onSave(idDark, color, instance));

		this.pickrDark.on("cancel", onPickrCancel);

		const themeDarkReset = new ButtonComponent(themeDarkWrapper.createDiv({cls: "pickr-reset"}));
